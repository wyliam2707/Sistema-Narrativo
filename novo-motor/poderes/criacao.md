# Criação [X]

Criação permite criar um objeto físico conforme a descrição do Poder.

## Hub

**Alcance:** Próximo `+0` → Curto `+1` → Médio `+2`

**Conjuração:** Cena `+0` → Turno `+1` → Imediata `+2`

**Objetos:** 1

**Tamanho:** Miúdo `+0` → Pequeno `+1` → Médio `+2` → Grande `+3` → Enorme `+4`

**Material:** Comum `+0` → Melhor `+1` → Aprimorado `+2`

**Duração:** Cena `+0` → Hora `+1` → Dia `+2` → Permanente `+3`

## Material

O Material define a resistência geral do objeto criado.

- **Comum — ND 1:** RD 2 e cerca de 10 PV.
- **Melhor — ND 2:** RD 3 e cerca de 20 PV.
- **Aprimorado — ND 3:** RD 4 e cerca de 30 PV.

Referência geral:

- **PV:** `10 × ND`
- **RD:** `ND + 1`

Os valores são uma referência média. A ficção, a forma e a natureza do objeto continuam determinando situações evidentes. O Tamanho define a dimensão narrativa do objeto e não altera, por si só, o ND do Material.

## Uso

O custo em Mana é a soma dos avanços escolhidos no Hub. As primeiras posições de cada progressão custam `+0`.

Se uma característica temporal não for determinada por outra regra, o padrão do sistema é instantâneo. Neste Poder, porém, Conjuração e Duração são escolhidas pelas progressões acima.

### Exemplo

Criar uma parede de **Tamanho Médio**, **Material Melhor** e **Alcance Médio**, usando **Conjuração Cena** e **Duração Cena**:

- Alcance Médio: `+2`
- Tamanho Médio: `+2`
- Material Melhor: `+1`
- Conjuração Cena: `+0`
- Duração Cena: `+0`

**Custo total:** 5 Mana.

A parede é de Material Melhor: **ND 2, RD 3 e cerca de 20 PV**.
