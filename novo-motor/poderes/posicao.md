...
Poder modular baseado na família nociva **Posição**.
...
**Dano:** Nenhum → 1d6 → 2d6  
**Efeito:** Posição: Caído → Movido → Conduzido  
...
- **Posição:** Caído `+0` → Movido `+1` → Conduzido `+2`
...
## Progressão de Posição

- **Caído:** o alvo fica no chão; deslocamento limitado a Próximo e `-1d` em ações físicas nas quais isso seja uma desvantagem relevante.
- **Movido:** desloca o alvo de forma simples para frente, para trás ou para um dos lados.
- **Conduzido:** desloca o alvo de forma controlada e precisa, definindo trajetória e ponto final dentro dos limites do efeito.

Conduzido altera apenas deslocamento e posição; não controla as demais ações do alvo.
